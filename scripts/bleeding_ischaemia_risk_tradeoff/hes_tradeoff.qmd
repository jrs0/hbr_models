...
for outcome, outcome_title in outcome_names.items():
  ...
  best_value_roc[outcome] = summary.iloc[best_for_roc]["ROC AUC"]
  best_value_calibration[outcome] = summary.iloc[best_for_calibration]["Cal. Error"]
  best_value_instability[outcome] = summary.iloc[best_for_instability]["Instability"]
  ...
  display(Markdown(
    ...
    f"{models[best_for_roc//2]} has the highest area under the ROC curve at {best_value_roc[outcome]:.3f}, "
    f"{models[best_for_calibration//2]} has the best calibration (error = {best_value_calibration[outcome]:.3f}), and "
    f"{models[best_for_instability//2]} has the highest stability (instability = {best_value_instability[outcome]:.3f}).")
  )
  best_model_roc[outcome] = models[best_for_roc//2]
  best_model_calibration[outcome] = models[best_for_calibration//2]
  best_model_instability[outcome] = models[best_for_instability//2]
```
...
In taking forward the results of this modelling to consider the development of a practical tool, the various hazardous situations that may arise from the use of the prediction tool must be enumerated, their risk evaluated, and risk control measures established to reduce the risk as much as possible [@iso14971]. Any subsequent benefit-risk analysis, which will compare the risks associated with using the models to the risks that are inherent in current clinical practice, will highly benefit from the numerical approaches to assessing model performance and robustness highlighted in this report.
