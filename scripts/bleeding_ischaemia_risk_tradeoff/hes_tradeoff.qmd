...
sns.displot(data=df, x="Age at index", hue="STEMI", bins=30, ax=ax).set(title='Proportion of STEMI and NSTEMI by age')
plt.show()
```
...
for m, (model, model_title) in enumerate(model_names.items()):
  ...
  plot_risk_tradeoff(model_name, bleeding_outcome, ischaemia_outcome):
```
